...
function NameSearch({ onNameSelect }) {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => fetchPopularByName(filmName),
  });
  ...
  return (
    <div ref={menuRef} className="flex items-center">
      <div className="w-80 bg-transparent py-2.5 px-5 border border-primary rounded-[25px] overflow-hidden">
        <input
          type="text"
          value={filmName}
          onChange={(e) => {
